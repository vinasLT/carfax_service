...
from rpc_client_server.health import HealthCheckServicer

sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'rpc_client_server', 'gen', 'python'))

from carfax.v1 import carfax_pb2_grpc, carfax_pb2
from rpc_client_server.carfax_server import CarfaxRpc
...
class GracefulServer:
    ...
    async def setup_server(self):
        ...
        carfax_pb2_grpc.add_CarfaxServiceServicer_to_server(CarfaxRpc(), self.server)
        health_pb2_grpc.add_HealthServicer_to_server(HealthCheckServicer(), self.server)

        if settings.ENVIRONMENT == Environment.DEVELOPMENT:
            try:
                service_names = [
                    carfax_pb2.DESCRIPTOR.services_by_name['CarfaxService'].full_name,
